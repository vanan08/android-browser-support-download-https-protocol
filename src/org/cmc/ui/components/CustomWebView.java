...
import org.cmc.utils.ProxySettings;
...
//		((MainActivity)mContext).setToolbarsVisibility(true);
		
		super.onScrollChanged(left, top, oldleft, oldtop);
		Log.d("CustomWebView", "top: "+top );
        if( top == 0 ){  // if diff is zero, then the bottom has been reached
            Log.d("CustomWebView", "MyScrollView: Bottom has been reached" );
            ((MainActivity)mContext).setToolbarsVisibility(true);
        }
...
//		int max = (int) Math.floor(this.getContentHeight() * this.getScale());
	...
	public void doOnResume() {
		if (mOnResumeMethod != null) {
			try {
				Log.i("CustomWebView", "doOnResume(): ");
	...
	public void doSetFindIsUp(boolean value) {
		if (mSetFindIsUp != null) {
			try {
				mSetFindIsUp.setAccessible(true);
	...
	private void loadMethods() {
		...
		if (!mBoMethodsLoaded) {
			...
			try {
				...
				Log.e("CustomWebView", "SecurityException loadMethods(): " + e.getMessage());
				mSetFindIsUp = null;
				mNotifyFindDialogDismissed = null;
			} catch (NoSuchMethodException e) {
				Log.e("CustomWebView", "NoSuchMethodException loadMethods(): " + e.getMessage());
				mSetFindIsUp = null;
				mNotifyFindDialogDismissed = null;
			}
